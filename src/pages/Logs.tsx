...
import { Search, Download, Filter, Eye, AlertTriangle, Info, CheckCircle, XCircle } from 'lucide-react';
...
export const Logs: React.FC = () => {
  ...
  const getActionIcon = (action: string) => {
    ...
    if (action.includes('FAILED') || action.includes('ERROR')) return <AlertTriangle className="h-4 w-4 text-red-600" />;
    if (action.includes('VALIDATE') || action.includes('APPROVE')) return <CheckCircle className="h-4 w-4 text-blue-600" />;
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-red-600">
              {allLogs.filter(log => log.action.includes('FAILED') || log.action.includes('ERROR')).length}
            </div>
          </CardContent>
        </Card>
        <Card>
                ...
                <TableRow key={log.id}>
                  <TableCell>
                    <div className="text-sm">
                      <div>{new Date(log.performedAt).toLocaleDateString('fr-FR')}</div>
                      <div className="text-muted-foreground">
                        {new Date(log.performedAt).toLocaleTimeString('fr-FR')}
                      </div>
                    </div>
                  </TableCell>
                  <TableCell>
