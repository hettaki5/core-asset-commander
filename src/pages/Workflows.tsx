...
import { GitBranch, Search, Filter, Eye, Clock, CheckCircle, ArrowRight, User } from 'lucide-react';
...
export const Workflows: React.FC = () => {
  ...
  const filteredHistory = allHistory.filter(item => {
    ...
    const matchesStatus = statusFilter === 'all' || item.toStatus === statusFilter;
    
    return matchesSearch && matchesEntity && matchesStatus;
  });

  const canViewWorkflows = ['admin', 'validateur'].includes(currentUser?.role || '');
  ...
  const getWorkflowStats = () => {
    ...
    const approvals = allHistory.filter(item => item.toStatus === 'approved').length;
    const rejections = allHistory.filter(item => item.toStatus === 'rejected').length;
        ...
        <Card className="w-96">
          <CardContent className="text-center py-8">
            ...
            <p className="text-muted-foreground">
              Seuls les administrateurs et validateurs peuvent accéder aux workflows.
            </p>
          </CardContent>
        </Card>
      </div>
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-muted-foreground">Suivez le cycle de vie des assets et tickets</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium">Transitions Total</CardTitle>
          </CardHeader>
          <CardContent>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-blue-600">{stats.todayTransitions}</div>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2">
      ...
      <Card>
        <CardHeader>
          <CardTitle>Historique des Transitions ({filteredHistory.length})</CardTitle>
          <CardDescription>
                  ...
                  <TableCell>
                    <div className="text-sm">
                      <div className="font-medium">
                        {new Date(item.performedAt).toLocaleDateString('fr-FR')}
                      </div>
                      <div className="text-muted-foreground">
                        {new Date(item.performedAt).toLocaleTimeString('fr-FR')}
                      </div>
                    </div>
                  </TableCell>
                  <TableCell>
        ...
        <CardContent>
          ...
          <div className="text-center mt-4">
            <p className="text-sm text-muted-foreground">
              Les assets suivent ce workflow de validation pour assurer la qualité des données
            </p>
          </div>
        </CardContent>
      </Card>
